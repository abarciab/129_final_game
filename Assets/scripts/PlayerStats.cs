...
public class PlayerStats : MonoBehaviour
{
    ...
    [SerializeField] SpriteRenderer swordSrend;
    [HideInInspector] public List<string> reactions;

    [HideInInspector] public Sprite idleSprite, attackSprite, hurtSprite, swordSprite;
    bool started;
    ...
    public void NextDialogue(InputAction.CallbackContext ctx) {
        if (ctx.started) GameManager.instance.NextDialogue(this);
    }

    public void PressPause(InputAction.CallbackContext ctx) {
        if (ctx.started) GameManager.instance.PauseGame();
    }
