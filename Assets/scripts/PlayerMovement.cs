...
public class PlayerMovement : MonoBehaviour {
    [SerializeField] float friction = 0.025f, maxSpeed;
    public float speed;
    Vector2 inputDir;
    Rigidbody2D rb;
    ...
    [SerializeField] float groundCheckRadius, jumpInputWindow, maxJumpUpTime;
    ...
    {
        if (ctx.started && !GameManager.instance.paused) { timeSinceJumpPressed = 0; jumpButtonDown = true; }
    ...
    public void Dash(float extraTime = 0)
    {
        timeSinceDashed = 0;
        StopAllCoroutines();
        StartCoroutine(DashCoroutine(extraTime));
    }

    IEnumerator DashCoroutine(float extraTime)
    {
        float time = 0;
        dashing = true;
