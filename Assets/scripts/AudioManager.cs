...
public class AudioManager : MonoBehaviour {
    ...
    public void StopSound(int ID, AudioSource source)
    {
        var clip = getClip(ID);
        if (source.clip == clip.clip) source.Stop();
    }
    public void PlaySoundNoInturrupt(int ID, AudioSource source)
    {
        var clip = getClip(ID);
        if (source.clip == clip.clip && source.isPlaying) return;
        ConfigureSource(clip, source);
        source.Play();
    }
